...
//            "</script>")
    ...
    @Select("SELECT * FROM calmwang_t_user user WHERE user.user_phone = #{phone}")
    List<CalmWangUserModel> findUserByPhone(@Param("phone") String phone);

    @Select("SELECT * FROM `calmwang_t_user` user WHERE user.user_phone = @{param1}")
    CalmWangUserModel getByPhone(String phone);
}
